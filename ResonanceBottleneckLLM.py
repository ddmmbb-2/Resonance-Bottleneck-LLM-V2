...
# ==========================================
# 🎯 V20 Phase 1.1 實驗配置 (節制版 - From Scratch)
# ==========================================
config = {
    "d_model": 512,          
    "n_heads": 8,            
    ...
    "save_model": "d2_v20_moderated.pth",   
    "log_csv": "v20_moderated_log.csv",     
    "vocab_name": "bpe_tokenizer_v12.json",     
    "vocab_size": 16384,                      
}

device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"🚀 V20 Phase 1.1 啟動 | 策略：白板節制 & 動態 Alpha | 設備: {device}")

# ==========================================
# 1. 資料加載
# ==========================================
if not os.path.exists(config["bin_data"]):
    raise FileNotFoundError(f"❌ 找不到 {config['bin_data']}！")
...
class ResonanceReasoningCoreV20(nn.Module):
    def __init__(self, d_model, latent_dim, think_steps=2):
        ...
        # 💡 [策略 1] 門控初始封印：讓門控預設關閉，強迫模型先學基礎語法
        nn.init.constant_(self.write_gate.bias, -3.0) 
        nn.init.constant_(self.read_gate.bias, -2.0)

        # 💡 [策略 2] 動態 Alpha：初始設為極低，由 Training Loop 監控 Loss 來打開
        self.register_buffer("work_alpha", torch.tensor(0.01))
    ...
    def _step(self, x, h_latent, workspace, step_idx):
        ...
        # 💡 [策略 3] 隨機失憶：強迫主幹網路具備獨立作戰能力
        if self.training:
            work_context = F.dropout(work_context, p=0.2)
...
class D2V20ModeratedModel(nn.Module):
...
if not os.path.exists(config["log_csv"]):
    with open(config["log_csv"], mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Step", "Loss", "LR", "Alpha", "Gate_Values"])
...
while global_step < config["epochs"]:
    ...
    gate_str = f"[{','.join([f'{g:.3f}' for g in gate_vals])}]"
    
    pbar.update(1)
    pbar.set_postfix({"Loss": f"{avg_loss:.3f}", "Alpha": f"{current_alpha:.3f}", "Gate": gate_str})
