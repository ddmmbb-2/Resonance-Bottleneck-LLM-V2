...
print(f"🔥 V22 Latent Optimizer 修復加速版啟動中 | 設備: {device}")
...
# ==========================================
# 4. V22 頂級加速交叉注意力 (FlashAttention-2 動態觸發版)
# ==========================================
class ReasonCrossAttention(nn.Module):
    ...
    def forward(self, h_query, K_mem, V_mem):
        ...
        # 將學習到的溫度融合至縮放因子，使 SDPA 自動熔合硬體加速
        scale_factor = torch.exp(self.temp * 0.5)
        Q = Q * scale_factor
        K = K * scale_factor
        
        # 直接調用 PyTorch 內置 FlashAttention 算子核心
...
while global_step < config["epochs"]:
    ...
    # 🌟 完美的延遲日誌機制：平時保持 N/A，只有每 10 步寫入時才去 GPU 取值，完全釋放流水線
