...
config = {
    ...
    "max_seq_len": 512,
    "batch_size": 4,
    "block_size": 384,         # 🚀 升級：受惠於 SSM 與 Latent 優化，擴充至 384
    "chunk_size": 384,          
    "cache_capacity": 512,     
    ...
    "log_csv": "v24_samba_latent_log.csv",   
...
print(f"🔥 V24 Samba-Latent Optimizer | 設備: {device}")
...
if not os.path.exists(config["log_csv"]):
    with open(config["log_csv"], mode='w', newline='') as f:
        writer = csv.writer(f)
        # 紀錄檔更新：移除 Step0_CE，加入 Align_Loss
        writer.writerow(["Step", "Final_CE", "Align_Loss", "Halt_Loss", "LR", "Diffs", "Halts"])

# ==========================================
# 2. 基礎組件 (RMSNorm, RoPE, SwiGLU)
# ==========================================
...
# ==========================================
# 5. V24 純潛在推理核心 (Pure Latent Optimizer)
# ==========================================
class ResonanceOptimizerCore(nn.Module):
    def __init__(self, d_model, latent_dim, think_steps=3):
        ...
        nn.init.constant_(self.master_gate.bias, -1.0)
    ...
    def forward(self, x, external_kv=None, return_all_steps=True):
        ...
        # 🚀 升級：不再紀錄投射回 d_model 的 intermediate_states，只紀錄純潛在狀態
        intermediate_latents = [] 
        diff_norms, halt_logits = [], []
        ...
        for i in range(self.steps):
            ...
            if return_all_steps:
                intermediate_latents.append(h_next)
        ...
        # 🚀 升級：只有最後一步才投射回主模型空間，大幅節省顯存與運算
        last_intermediate = self.latent_to_model(h_next)
        
        if return_all_steps:
            return last_intermediate, intermediate_latents, diff_norms, halt_logits
        else:
            return last_intermediate, [], diff_norms, halt_logits
...
class D2V24HybridModel(nn.Module):
    def __init__(self, vocab_size, d_model, n_layers):
        ...
        # 🚀 升級：Samba 式層疊 (1 Attention + 3 SSM)
        # 注意力層：0, 4, 8
        # SSM層：1, 2, 5, 6, 9, 10
        # 推理層：3, 7, 11 (兼具上下文摘要整合功能)
    ...
    def forward(self, x, return_all_steps=False):
        ...
        all_diffs_chunked = []
        all_halts_chunked = []
        step_latents_chunked = [] # 儲存潛在狀態而非 logits
        ...
        for start in range(0, L, chunk_size):
            ...
            for idx, block in enumerate(self.blocks):
                if isinstance(block, ResonanceOptimizerCore):
                    out, latents, diffs, halts = block(curr_x, external_kv=ext_kv, return_all_steps=return_all_steps)
                    curr_x = curr_x + out
                    
                    # 🚀 升級：只收集最後一個推理核心的潛在狀態作為監督訊號，不再進行深層算圖展開
            ...
            if return_all_steps:
                step_latents_chunked.append(chunk_latents)
        ...
        if return_all_steps:
            # 依步驟拼接每個 chunk 的潛在狀態
            num_steps = len(step_latents_chunked[0])
            final_step_latents = []
            final_diffs = []
            final_halts = []
            
            for s in range(num_steps):
                final_step_latents.append(torch.cat([chk[s] for chk in step_latents_chunked], dim=1))
            ...
            return final_logits, final_step_latents, final_diffs, final_halts
...
while global_step < config["epochs"]:
    ...
    step_final_ce = 0
    step_align_loss_total = 0
    step_halt_loss_total = 0 
    ...
    avg_align = step_align_loss_total / config["accum_steps"]
    avg_halt = step_halt_loss_total / config["accum_steps"]
