...
config = {
    ...
    # 🎯 Phase 2 影子門控超參數
    "halt_tau": 0.05,                  
    "halt_weight": 0.5,                # ⚖️ 提升輔助 Loss 權重，強迫模型重視
...
while global_step < config["epochs"]:
    ...
    # 這裡不再需要關閉 halt_weight，直接火力全開
    current_halt_weight = config["halt_weight"]
    ...
    if global_step % 10 == 0:
        with open(config["log_csv"], mode='a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([global_step, f"{avg_ce:.4f}", f"{avg_halt:.4f}", f"{optimizer.param_groups[0]['lr']:.6f}", diff_str, halt_str])
